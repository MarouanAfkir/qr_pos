...
.cat-nav{
    ...
    background:#fff;border:1px solid #facc15;
    box-shadow:0 2px 6px rgba(0,0,0,.12);
    color:#7a5200;font-size:.85rem;z-index:5;transition:opacity .15s;
}
.cat-nav.disabled{display:none}      /* hide arrow when not needed */
.cat-prev{left:.25rem}
.cat-next{right:.25rem}

.categories-scroll{
    display:flex;flex-wrap:nowrap;gap:.5rem;
    margin-bottom:.55rem;padding:0 1.6rem;
...
.categories-scroll .nav-link{
    padding:.65rem 1.15rem;font-size:.95rem;font-weight:600;
    color:#7a5200;background:linear-gradient(145deg,#fffbe8 0%,#fef6c9 100%);
    border:1px solid rgba(250,204,21,.45);
    border-radius:14px 14px 0 14px;
    transition:.2s box-shadow,.2s transform;
    line-height:1.1;
}
.categories-scroll .nav-link:hover,
.categories-scroll .nav-link:focus{transform:translateY(-2px);box-shadow:0 4px 10px rgba(0,0,0,.08)}
.categories-scroll .nav-link.active{
    color:#000;background:linear-gradient(135deg,#facc15 0%,#fde047 100%);
    border-color:transparent;box-shadow:0 4px 12px rgba(0,0,0,.1)
}

/* ≥576px centre list & hide buttons */
...
<header class="restaurant-header food-menu-section fix position-relative">
    ...
    <div class="position-absolute top-0 end-0 pe-4 pt-4 lang-switcher">
        <div class="dropdown">
            <button class="btn btn-sm rounded-pill d-flex align-items-center gap-1" data-bs-toggle="dropdown"><i class="fa-solid fa-globe"></i>{{ strtoupper($default_language) }}</button>
            <ul class="dropdown-menu dropdown-menu-end shadow-sm fade">
                @foreach($languages ?? [] as $lang)
                    <li><a class="dropdown-item {{ $lang['code']==$default_language?'active fw-bold':'' }}" href="{{ request()->fullUrlWithQuery(['lang'=>$lang['code']]) }}">{{ strtoupper($lang['code']) }} — {{ $lang['name'] }}</a></li>
                @endforeach
            </ul>
        </div>
    </div>
    ...
    <div class="food-menu-wrapper style1">
        <div class="container text-center">
            <div class="restaurant-logo-wrapper"><img src="{{ $logo?asset($logo):asset('assets/img/logo/logo.svg') }}" alt="Logo"></div>
            ...
            <div class="header-meta d-flex flex-wrap justify-content-center gap-3 mt-3">
                @isset($restaurantAddress)<span><i class="fa-solid fa-location-dot me-1"></i>{{ $restaurantAddress }}</span>@endisset
                @isset($phone1)<span><i class="fa-solid fa-phone-volume me-1"></i>{{ $phone1 }}</span>@endisset
                @isset($phone2)<span><i class="fa-solid fa-phone-volume me-1"></i>{{ $phone2 }}</span>@endisset
            </div>
        ...
        <div class="container ">
            <div class="food-menu-tab-wrapper style-bg pt-5">
                ...
                <div class="row justify-content-center mb-4">
                    <div class="col-md-6"><input id="menuSearch" type="search" class="form-control shadow-sm" placeholder="{{ __('Search menu…') }}"></div>
                </div>
                ...
                <div class="food-menu-tab">
                    ...
                    <div class="categories-wrapper">
                        <button type="button" class="cat-nav cat-prev"><i class="fa-solid fa-chevron-left"></i></button>
                        ...
                        <button type="button" class="cat-nav cat-next"><i class="fa-solid fa-chevron-right"></i></button>
                    </div>
                    ...
                    <div class="tab-content" id="pills-tabContent">
                        @foreach($categories as $cat)
                            ...
                            @endphp
                            <div class="tab-pane fade {{ $loop->first?'show active':'' }}" id="pills-{{ $slug }}" role="tabpanel">
                                <div class="row gx-40">
                                            ...
                                            @foreach($colItems as $item)
                                                @php
                                                    ...
                                                    $priceHtml = $salePrice
                                                        ...
                                                        : $price.$currency;
                                                @endphp
                                                <div class="single-menu-items" data-name="{{ e($item['name']) }}"
                                                     data-desc="{{ e($item['description'] ?? '') }}"
                                                     data-img="{{ $img }}"
                                                     data-price="{!! $priceHtml !!}">
                                                    <div class="menu-item-thumb"><img src="{{ $img }}" alt="{{ $item['name'] }}" loading="lazy"></div>
                                                    <div class="menu-content flex-grow-1">
                                                        <h3 class="mb-1 fw-semibold">{{ $item['name'] }}</h3>
                                                        @if($item['description'])<p class="mb-1 small text-muted">{{ $item['description'] }}</p>@endif
                                                    </div>
...
<div class="modal fade" id="itemModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            <div class="modal-header"><h4 class="modal-title fw-semibold"></h4><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
...
$('#menuSearch').on('input',function(){
    ...
    const $pills=$('#pills-tab'),$panes=$('#pills-tabContent'),$res=$('#searchResults').empty();
    if(!q){$res.addClass('d-none');$pills.removeClass('d-none');$panes.removeClass('d-none');return;}
    $pills.addClass('d-none');$panes.addClass('d-none');$res.removeClass('d-none');
    $('.single-menu-items').each(function(){
        const $it=$(this),n=($it.data('name')||'').toLowerCase(),d=($it.data('desc')||'').toLowerCase();
        if(n.includes(q)||d.includes(q)) $('<div class="col-lg-6 col-md-6"></div>').append($it.clone()).appendTo($res);
    });
    if(!$res.children().length) $res.html('<p class="text-center py-4 text-muted">{{ __('No items match your search.') }}</p>');
});
</script>
</body>
</html>
