...
<head>
    ...
    <style>
        ...
        #searchResults .single-menu-items {
            margin-bottom: .85rem
        }
        ...
        .categories-scroll .nav-link {
            ...
            font-weight: 600;
            color: #7a5200;
            background: linear-gradient(145deg, #fffbe8 0%, #fef6c9 100%);
        ...
        .categories-scroll .nav-link:focus {
            transform: translateY(-2px);
...
<body class="bg-color2">
    ...
    <header class="restaurant-header food-menu-section fix position-relative">
        ...
        <div class="position-absolute top-0 end-0 pe-4 pt-4 lang-switcher">
            <div class="dropdown">
                <button class="btn btn-sm rounded-pill d-flex align-items-center gap-1" data-bs-toggle="dropdown"><i
                        class="fa-solid fa-globe"></i>{{ strtoupper($default_language) }}</button>
                <ul class="dropdown-menu dropdown-menu-end shadow-sm fade">
                    @foreach ($languages ?? [] as $lang)
                        <li><a class="dropdown-item {{ $lang['code'] == $default_language ? 'active fw-bold' : '' }}"
                                href="{{ request()->fullUrlWithQuery(['lang' => $lang['code']]) }}">{{ strtoupper($lang['code']) }}
                                — {{ $lang['name'] }}</a></li>
                    @endforeach
                </ul>
            </div>
        ...
        <div class="food-menu-wrapper style1">
            <div class="container text-center">
                <div class="restaurant-logo-wrapper"><img
                        src="{{ $logo ? asset($logo) : asset('assets/img/logo/logo.svg') }}" alt="Logo"></div>
            ...
            <div class="container ">
                <div class="food-menu-tab-wrapper style-bg pt-5 px-1 px-md-5">
                    ...
                    <div class="row justify-content-center mb-4">
                        <div class="col-md-6"><input id="menuSearch" type="search" class="form-control shadow-sm"
                                placeholder="{{ __('Search menu…') }}"></div>
                    </div>
                                            ...
                                            <div class="col-lg-6 col-md-6">
                                                @foreach ($colItems as $item)
                                                    @php
                                                        ...
                                                        $img =
                                                            $item['image'] ?? asset('assets/img/menu/menuThumb1_1.png');
                                                        $priceHtml = $salePrice
                                                            ? '<del class=&quot;text-muted me-1&quot;>' .
                                                                $price .
                                                                $currency .
                                                                '</del> <span class=&quot;text-theme-color2 fw-semibold&quot;>' .
                                                    ...
                                                    <div class="single-menu-items" data-name="{{ e($item['name']) }}"
                                                        ...
                                                        <div class="menu-item-thumb"><img src="{{ $img }}"
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title fw-semibold"></h4><button type="button" class="btn-close"
    ...
    <script>
        ...
        prevBtn.addEventListener('click', () => catStrip.scrollBy({
            left: -200,
            behavior: 'smooth'
        }));
        nextBtn.addEventListener('click', () => catStrip.scrollBy({
            left: 200,
            behavior: 'smooth'
        }));
        ...
        $('#menuSearch').on('input', function() {
            ...
            $('.single-menu-items').each(function() {
                ...
                if (n.includes(q) || d.includes(q)) $('<div class="col-lg-6 col-md-6"></div>').append($it
                    .clone()).appendTo($res);
            });
            if (!$res.children().length) $res.html(
                '<p class="text-center py-4 text-muted">{{ __('No items match your search.') }}</p>');
        });
    </script>
</body>
