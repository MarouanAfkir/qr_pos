...
@php
    ...
    $taxRate     = (float)($restaurant['settings']['tax_rate'] ?? 0);      // e.g., 10 means 10%
    $serviceRate = (float)($restaurant['settings']['service_rate'] ?? 0);  // e.g., 5
...
<head>
    ...
    <style>
        ...
        *{box-sizing:border-box}
        ...
        .logo-ring{
            width:56px; height:56px; border-radius:50%; background:#fff; border:3px solid #f6efe6;
        ...
        /* Language switcher */
        .btn-pill {
            background:#fff; border:1px solid #e7d9c8; color:var(--ink);
        ...
        .pos-header .dropdown-item:hover{background:rgba(196,154,108,.12)}
        .pos-header .dropdown-item.active{background:linear-gradient(135deg,#fde7c7,#fff); font-weight:800}

        /* Header table toggle */
        .btn-circle {
            width:40px;height:40px;border-radius:50%;border:1px solid #e7d9c8;background:#fff;
            display:inline-flex;align-items:center;justify-content:center;
        }
        .table-badge {
            background:#111827; color:#fff; border-radius:6px; padding:.1rem .4rem; font-size:.7rem; font-weight:800
        }

        /* ===== Shell layout ===== */
        .pos-shell{
            display:grid; gap:1rem; grid-template-columns: 1fr 390px;
        ...
        .choice-cards{
            display:grid; gap:.5rem; grid-template-columns: repeat(3, minmax(0,1fr)); margin-bottom:.6rem;
        }
        @media (max-width:576px){ .choice-cards{grid-template-columns:1fr 1fr 1fr} }
        .choice-card{
            border:2px solid #eadfd3; background:#fff; border-radius:.9rem; padding:.6rem .7rem;
        ...
        .choice-ico{
            width:36px;height:36px;border-radius:10px;background:#fff7ea;border:1px solid #f2e4d3;
            display:flex;align-items:center;justify-content:center;color:#6b4b2f;font-size:1rem;flex-shrink:0
        }
        ...
        .pos-search{
            border:2px solid #eadfd3; border-radius:999px; padding:.55rem .9rem; min-width:280px; flex:1;
        }
        ...
        .import-wrap .form-control{border-radius:.6rem; border:1px solid #e6dbcf; height:40px; font-size:.92rem; min-width:230px}
        .btn-sub{border:1px solid #eadfd3; background:#fff; border-radius:.7rem; font-weight:700; padding:.45rem .7rem}

        /* ===== Categories ===== */
        .categories-scroll{
            display:flex; gap:.5rem; flex-wrap:nowrap; overflow:auto; scrollbar-width:none; -ms-overflow-style:none; padding:.5rem .2rem 0;
        }
        .categories-scroll::-webkit-scrollbar{display:none}
        .categories-scroll .btn{
            border-radius:999px; border:1px solid #eadfd3; background:#fff; color:#5a3d1b; font-weight:700; padding:.35rem .85rem;
        }
        .categories-scroll .btn.active{
            background:#f6e3cf; border-color:#e2c9a7; color:#1f2937;
            box-shadow:0 4px 10px rgba(0,0,0,.08)
        }

        /* ===== Items grid ===== */
        .items-grid{
            display:grid; gap:.7rem; grid-template-columns:repeat(auto-fill,minmax(170px,1fr));
            margin-top:.6rem;
        }
        .item-card{
        ...
        .item-thumb{height:110px; background:#f2f2f2}
        ...
        .cart-line{
            display:grid; grid-template-columns: 1fr auto; gap:.4rem; border-bottom:1px dashed #eadfd3; padding:.55rem 0;
        }
        .cart-line:last-child{border-bottom:none}
        .cart-line h6{margin:0; font-weight:800}
        .opts small{color:var(--muted)}
        .qty-ctrl{display:flex; align-items:center; gap:.35rem}
        .qty-ctrl button{width:26px;height:26px;border:1px solid #eadfd3;background:#fff7ea;border-radius:6px}
        .line-total{font-weight:800}
        .cart-foot{padding:.75rem .9rem; border-top:1px solid var(--line); background:#fffaf3}
        ...
        .btn-main{
            background:var(--brand); color:#fff; border:none; border-radius:.7rem; font-weight:800;
            padding:.6rem .9rem; box-shadow:0 10px 22px rgba(111,78,55,.22);
        }
        .btn-main:hover{background:#5a3e2f}
        .btn-danger-soft{border:1px solid #fde2e2; background:#fff5f5; color:#991b1b}

        .parked{border-top:1px dashed #eadfd3; margin-top:.6rem; padding-top:.6rem}
        .order-chip{
            display:flex; align-items:center; justify-content:space-between; gap:.5rem;
            border:1px dashed #eadfd3; border-radius:.65rem; padding:.35rem .5rem; background:#fff;
            margin-bottom:.45rem;
        }
        ...
        .modal-header{background:#fff3cf; border-bottom:none}
        .modal-title{font-weight:800; color:#5f3b0e}
        ...
        .qty-btn{width:32px; height:32px; display:flex; align-items:center; justify-content:center; border-radius:8px; border:1px solid #f2e4d3; background:#fff7ea}
        .qty-input{width:58px; text-align:center; border:1px solid #f2e4d3; background:#fff; border-radius:8px; padding:.32rem 0}
        .req-hint{font-size:.9rem; color:#b45309; display:none}

        /* ===== Payment & Scanner ===== */
        .pay-methods .btn{border:1px solid #eadfd3; background:#fff; padding:.5rem .75rem; border-radius:.7rem; font-weight:700}
        ...
        .table-card{
            border:2px solid #eadfd3; background:#fff; border-radius:.9rem; padding:.7rem .6rem; text-align:center; cursor:pointer; user-select:none; transition:.15s;
        }
        ...
        .table-ico{width:38px;height:38px;border-radius:10px;background:#fff7ea;border:1px solid #f2e4d3;display:flex;align-items:center;justify-content:center;color:#6b4b2f;margin:0 auto .35rem}
...
<body>
            ...
            <div class="d-flex align-items-center justify-content-between">
                <div class="brand-wrap">
                    ...
                    <div class="brand-title">
                        <h1>{{ $restaurantName }} <span class="badge-pill ms-1" style="background:#fff; border:1px solid #eadfd3; font-size:.7rem;">POS</span></h1>
                        <small>{{ $tagline }}</small>
                    </div>
                </div>
                ...
                <div class="header-tools">
                    ...
                    {{-- Table toggle --}}
                    <button class="btn-circle" id="openTables" title="{{ __('Choose Table') }}" data-bs-toggle="offcanvas" data-bs-target="#tablesCanvas" aria-controls="tablesCanvas">
    ...
    <div class="container-fluid pos-shell">
        ...
        <section class="panel">
            <div class="panel-head">
                ...
                {{-- 2) Search + Client import --}}
                <div class="search-row">
                    <input id="posSearch" type="search" class="pos-search" placeholder="{{ __('Search items…') }}" aria-label="{{ __('Search items…') }}">
                    <div class="import-wrap">
                        <input id="clientCode" class="form-control" placeholder="{{ __('Client Code / URL / JSON') }}">
                        <button class="btn btn-sub" id="importCode" title="{{ __('Import client order') }}"><i class="fa-solid fa-download"></i></button>
                        <button class="btn btn-sub" id="scanCode" data-bs-toggle="modal" data-bs-target="#scanModal" title="{{ __('Scan QR') }}"><i class="fa-solid fa-qrcode"></i></button>
                    </div>
                </div>
                ...
                <div class="categories-scroll" id="catPills">
                    ...
                    @foreach ($categories as $cat)
                        ...
                        <button class="btn" data-cat="{{ $slug }}">{{ $cat['name'] }}</button>
                    @endforeach
                </div>
            </div>
                            ...
                            >
                                ...
                                <div class="item-meta">
                                    ...
                                    <div class="d-flex align-items-center justify-content-end">
                                        <div class="price-tag">{{ number_format($base,2) . $currency }}</div>
                                    </div>
                                </div>
                            </div>
                        @endforeach
        ...
        <aside class="panel cart">
            <div class="cart-head">
                <div class="fw-bold">
                    ...
                    <span class="badge-pill ms-1" style="background:#fff; border:1px solid #eadfd3">{{ __('Type') }}: <span id="orderTypeBadge">DINE-IN</span></span>
                    <span class="badge-pill ms-1" style="background:#fff; border:1px solid #eadfd3">{{ __('Table') }}: <span id="cartTableBadge">—</span></span>
                </div>
                <div class="d-flex gap-1">
                    <button class="btn btn-sub btn-sm" id="holdOrder"><i class="fa-solid fa-box-archive me-1"></i>{{ __('Hold') }}</button>
                    <button class="btn btn-danger-soft btn-sm" id="clearCart"><i class="fa-solid fa-trash-can me-1"></i>{{ __('Clear') }}</button>
                </div>
            </div>
            ...
            <div class="cart-foot">
                ...
                @if($serviceRate>0)
                <div class="tot-row"><span>{{ __('Service') }} ({{ $serviceRate }}%)</span><span id="t_srv">0{{ $currency }}</span></div>
                @endif

                @if($taxRate>0)
                <div class="tot-row"><span>{{ __('Tax') }} ({{ $taxRate }}%)</span><span id="t_tax">0{{ $currency }}</span></div>
                @endif
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="row g-2 align-items-end">
                        ...
                        <div class="col-6">
                            <div class="change-box"><i class="fa-solid fa-coins me-1"></i>{{ __('Change') }}: <span id="payChange">0{{ $currency }}</span></div>
                        </div>
                    </div>
    ...
    <div class="modal fade" id="scanModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="text-muted small mb-2">{{ __('When detected, items will be added to the current order automatically.') }}</div>
                    <div id="qrReader"></div>
                </div>
            </div>
        </div>
    ...
    <div class="offcanvas offcanvas-end" tabindex="-1" id="tablesCanvas" aria-labelledby="tablesCanvasLabel">
        ...
        <div class="offcanvas-body">
            <div id="tablesNotice" class="alert alert-warning py-2 px-3 d-none">
                <i class="fa-solid fa-circle-info me-1"></i> {{ __('Tables can be assigned for Dine-in orders only.') }}
            </div>
    ...
    <script>
        /* ========= Globals ========= */
        const CURR = '{{ $currency }}';
        const TAX  = {{ $taxRate }};     // percent
        const SRV  = {{ $serviceRate }}; // percent
        ...
        document.querySelectorAll('#orderTypeCards .choice-card').forEach(card=>{
            ...
            card.addEventListener('keydown', (e)=>{ if(e.key==='Enter' || e.key===' ') { e.preventDefault(); setOrderType(card.getAttribute('data-type')); }});
        });
        setOrderType('dinein');
        ...
        function enforceMax(group, changedInput){
            ...
            if(checked.length > max){
                ...
                group.style.transform='scale(1.01)'; setTimeout(()=>group.style.transform='',120);
            }
        }
        ...
        $('#cfgMinus').on('click', ()=>{
            ...
            let v = parseInt(inp.value||min,10); v = Math.max(min, v-1);
            inp.value=v;
        });
        $('#cfgPlus').on('click', ()=>{
            const inp = document.getElementById('cfgQty');
            const max = parseInt(inp.max||0,10) || Infinity;
            let v = parseInt(inp.value||1,10); v = Math.min(max, v+1);
            inp.value=v;
        });
        ...
        function renderCart(){
            ...
            let discount = 0;
            if(discTypePct){ discount = sub * (discVal/100); } else { discount = discVal; }
            discount = Math.max(0, Math.min(discount, sub));

            let srv = SRV > 0 ? (sub - discount) * (SRV/100) : 0;
            let tax = TAX > 0 ? (sub - discount + srv) * (TAX/100) : 0;
            let grand = sub - discount + srv + tax;

            document.getElementById('t_sub').innerText = money(sub);
            const srvEl = document.getElementById('t_srv'); if(srvEl) srvEl.innerText = money(srv);
            const taxEl = document.getElementById('t_tax'); if(taxEl) taxEl.innerText = money(tax);
            ...
            list.querySelectorAll('button[data-act]').forEach(b=>{
                b.addEventListener('click', ()=>{
                    ...
                    if(b.getAttribute('data-act')==='resume'){
                        ...
                        updateTableBadges();
                        arr.splice(idx,1); setParked(arr); renderParked(); renderCart();
                    }else{
                        arr.splice(idx,1); setParked(arr); renderParked();
                    }
                });
            });
        }
        ...
        document.getElementById('holdOrder').addEventListener('click', ()=>{
            ...
            const srv = SRV>0 ? (sub-discAmt)*(SRV/100):0;
            const tax = TAX>0 ? (sub-discAmt+srv)*(TAX/100):0;
            const total = sub-discAmt+srv+tax;
            ...
            const arr = getParked(); arr.unshift(ord); setParked(arr); renderParked();
        ...
        document.getElementById('clearCart').addEventListener('click', ()=>{
            cartLines.splice(0,cartLines.length); renderCart();
        });
        ...
        $('#checkoutBtn').on('click', ()=>{
            ...
            $('#payGiven').val('0'); $('#payChange').text(money(0));
            payModal.show();
        });
        ...
        function finalize(printIt){
            ...
            if(printIt){
                ...
                w.document.write(`<div>Order: ${summary.id}</div><div class="small">${new Date(summary.time).toLocaleString()}</div><div class="small">Type: ${summary.type.toUpperCase()}${summary.table? ' • Table '+summary.table:''}</div><hr>`);
        ...
        function tryParsePayload(raw){
            if(!raw) return null;
            try{ if(/^https?:\/\//i.test(raw)){ const u = new URL(raw); const p = u.searchParams.get('data'); if(p) raw = p; } }catch(e){}
            try{ return JSON.parse(raw); }catch(e){}
            try{ const txt = atob(raw); return JSON.parse(txt);}catch(e){}
            return null;
        }
        function importClientOrder(payload){
            if(!payload || !Array.isArray(payload.items)){ alert('{{ __("Invalid client code") }}'); return false; }
        ...
        $('#importCode').on('click', ()=>{
            const raw = ($('#clientCode').val()||'').trim();
            const payload = tryParsePayload(raw);
            if(importClientOrder(payload)){ $('#clientCode').val(''); }
        });
            ...
            if(window.Html5QrcodeScanner){
                ...
                qrScanner.render((decodedText)=>{
                    ...
                    if(importClientOrder(payload)){
                        const m = bootstrap.Modal.getInstance(scanModalEl); m?.hide();
        ...
        document.getElementById('clearTable').addEventListener('click', ()=>{
            currentTable = null; updateTableBadges();
        });

        document.querySelectorAll('.table-card').forEach(card=>{
            card.addEventListener('click', ()=>{
                const t = card.getAttribute('data-table');
                currentTable = t; updateTableBadges();
            });
