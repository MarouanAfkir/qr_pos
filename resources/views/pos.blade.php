...
<head>
    ...
    <style>
        ...
        * {
        ...
        .pos-header .dropdown-item.active {
            background: linear-gradient(135deg, #fde7c7, #fff);
            font-weight: 800
        }
        ...
        /* Show the whole image (letterboxed if needed) */
        .item-thumb {
            height: 110px;
            background: #fff;
            /* neutral background behind letterboxing */
            display: flex;
            align-items: center;
            justify-content: center;
            overflow: hidden;
        ...
        .btn-main {
            ...
            font-weight: 800;
            padding: .6rem .9rem;
            box-shadow: 0 10px 22px rgba(111, 78, 55, .22);
        }
...
<body>
                ...
                <div class="brand-wrap">
                    ...
                    <div class="brand-title">
                        <h1>{{ $restaurantName }} <span class="badge-pill ms-1"
                                style="background:#fff; border:1px solid #eadfd3; font-size:.7rem;">POS</span></h1>
                        <small>{{ $tagline }}</small>
                    </div>
                </div>
    ...
    <div class="container-fluid pos-shell">
        ...
        <section class="panel">
            <div class="panel-head">
                ...
                <div class="choice-cards" id="orderTypeCards">
                    <div class="choice-card active" data-type="dinein" tabindex="0" role="button"
                        aria-pressed="true">
                ...
                <div class="search-row">
                    ...
                    <div class="import-wrap">
                        ...
                        <button class="btn btn-sub" id="importCode" title="{{ __('Import client order') }}"><i
                ...
                <div class="items-grid" id="itemsGrid">
                    @foreach ($categories as $cat)
                        ...
                        @foreach ($cat['items'] as $item)
                            ...
                            @endphp
                            <div class="item-card" data-cat="{{ $slug }}"
                                data-name="{{ e($item['name']) }}" data-desc="{{ e($item['description'] ?? '') }}"
                                data-img="{{ $img }}" data-base="{{ number_format($base, 2, '.', '') }}"
                                data-minqty="{{ (int) $minQty }}" data-maxqty="{{ $maxQty ? (int) $maxQty : '' }}"
                                data-variations='@json($item['variations'] ?? [])'>
                                <div class="item-thumb"><img src="{{ $img }}" alt="{{ $item['name'] }}">
                                </div>
                                <div class="item-meta">
        ...
        <aside class="panel cart">
            <div class="cart-head">
                <div class="fw-bold">
                    <i class="fa-solid fa-basket-shopping me-1"
                        style="color:var(--brand)"></i>{{ __('Current Order') }}
                    <span class="badge-pill ms-1"
                        style="background:#fff; border:1px solid #eadfd3">{{ __('Type') }}: <span
                            id="orderTypeBadge">DINE-IN</span></span>
                    <span class="badge-pill ms-1"
                        style="background:#fff; border:1px solid #eadfd3">{{ __('Table') }}: <span
                            id="cartTableBadge">—</span></span>
                </div>
            ...
            <div class="cart-foot">
                <div class="tot-row"><span>{{ __('Subtotal') }}</span><strong
                        id="t_sub">0{{ $currency }}</strong></div>

                <div class="tot-row align-items-center">
                    <div class="d-flex align-items-center gap-2">
                        <span>{{ __('Discount') }}</span>
                        <div class="btn-group btn-group-sm" role="group">
                            <input type="radio" class="btn-check" name="discType" id="discAmt"
                                autocomplete="off" checked>
                            <label class="btn btn-outline-secondary" for="discAmt">{{ __('Amount') }}</label>
                            <input type="radio" class="btn-check" name="discType" id="discPct"
                                autocomplete="off">
                            <label class="btn btn-outline-secondary" for="discPct">%</label>
                        </div>
                    </div>
                    <div>
                        <input id="discVal" type="number" class="form-control form-control-sm" step="0.01"
                            value="0" style="width:110px; text-align:right">
                    </div>
                </div>

                @if ($serviceRate > 0)
                    <div class="tot-row"><span>{{ __('Service') }} ({{ $serviceRate }}%)</span><span
                            id="t_srv">0{{ $currency }}</span></div>
                @endif

                @if ($taxRate > 0)
                    <div class="tot-row"><span>{{ __('Tax') }} ({{ $taxRate }}%)</span><span
                            id="t_tax">0{{ $currency }}</span></div>
                @endif

                <div class="tot-row grand"><span>{{ __('Grand Total') }}</span><span
                        id="t_grand">0{{ $currency }}</span></div>

                <div class="d-grid gap-2 mt-3">
                    <button class="btn-main" id="checkoutBtn"><i
                            class="fa-solid fa-cash-register me-1"></i>{{ __('Charge Payment') }}</button>
                ...
                <div class="parked">
                    <div class="fw-bold mb-2"><i
                            class="fa-solid fa-clock-rotate-left me-1"></i>{{ __('Held Orders') }}</div>
                        ...
                        <div class="flex-grow-1">
                            ...
                            <div class="d-flex align-items-center justify-content-between mt-2">
                                <div class="qty-wrap">
                                    ...
                                    <input id="cfgQty" class="qty-input" type="number" value="1"
                                        min="1" step="1">
                                    <button class="qty-btn" id="cfgPlus"><i class="fa-solid fa-plus"></i></button>
                                </div>
                                <div class="req-hint" id="cfgReq">{{ __('Please complete required selections.') }}
                                </div>
                            </div>

                            <div class="d-flex align-items-center justify-content-between mt-3">
                                <button id="cfgAdd" class="btn-main"><i
    ...
    <div class="modal fade" id="payModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title"><i class="fa-solid fa-cash-register me-2"></i>{{ __('Take Payment') }}
                    </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"
                        aria-label="{{ __('Close') }}"></button>
                </div>
                <div class="modal-body">
                    <div class="mb-2">{{ __('Grand Total') }}: <strong
                            id="payTotal">0{{ $currency }}</strong></div>
                    <div class="pay-methods d-flex gap-2 mb-2">
                        <button class="btn btn-sm active" data-pay="cash"><i
                                class="fa-solid fa-money-bill-wave me-1"></i>{{ __('Cash') }}</button>
                        <button class="btn btn-sm" data-pay="card"><i
                                class="fa-solid fa-credit-card me-1"></i>{{ __('Card') }}</button>
                        <button class="btn btn-sm" data-pay="other"><i
                                class="fa-solid fa-wallet me-1"></i>{{ __('Other') }}</button>
                    </div>
                    <div class="row g-2 align-items-end">
                        <div class="col-6">
                            <label class="form-label small">{{ __('Amount Tendered') }}</label>
                            <input id="payGiven" type="number" step="0.01" class="form-control"
                                value="0">
                        </div>
                        <div class="col-6">
                            <div class="change-box"><i class="fa-solid fa-coins me-1"></i>{{ __('Change') }}: <span
                                    id="payChange">0{{ $currency }}</span></div>
                        </div>
                    </div>
                    <div class="mt-3 d-grid gap-2">
                        <button id="finalizePayment" class="btn-main"><i
                                class="fa-solid fa-check me-1"></i>{{ __('Finalize & Print') }}</button>
                        <button id="finalizeNoPrint" class="btn-sub"><i
                                class="fa-solid fa-check me-1"></i>{{ __('Finalize (No Print)') }}</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="offcanvas offcanvas-end" tabindex="-1" id="tablesCanvas" aria-labelledby="tablesCanvasLabel">
        ...
        <div class="offcanvas-body">
            ...
            <div class="table-grid" id="tablesGrid">
                @for ($i = 1; $i <= 40; $i++)
                    <div class="table-card" data-table="{{ $i }}" role="button" tabindex="0"
                        aria-label="{{ __('Table') }} {{ $i }}">
        ...
        <div class="offcanvas-footer">
            <div class="d-flex justify-content-between">
                <button class="btn btn-danger-soft" id="clearTable"><i
                        class="fa-solid fa-xmark me-1"></i>{{ __('Clear Table') }}</button>
    ...
    <script>
        ...
        function money(n) {
            return (parseFloat(n || 0).toFixed(2)) + CURR;
        }
        ...
        document.querySelectorAll('#orderTypeCards .choice-card').forEach(card => {
            ...
            card.addEventListener('keydown', (e) => {
                if (e.key === 'Enter' || e.key === ' ') {
                    e.preventDefault();
                    setOrderType(card.getAttribute('data-type'));
                }
            });
        });
        setOrderType('dinein');
        ...
        function renderVariations(variations) {
            ...
            variations.forEach((v, i) => {
                ...
                if (!single) {
                    html +=
                        `<div class="option-hint">{{ __('You can select up to') }} ${maxSel} {{ __('option(s)') }}</div>`;
                }
                html += `</div>`;
            });
            return html;
            ...
            document.querySelectorAll('#cfgOptions .v-group').forEach(g => {
                ...
                if (defs.length) {
                    if (single) {
                        ...
                        defs.forEach(d => d.checked = true);
                    }
                } else {
                    const min = parseInt(g.getAttribute('data-min') || 0, 10);
                    if (min > 0 && inputs.length === 1) {
        ...
        function gatherSelections() {
            ...
            document.querySelectorAll('#cfgOptions .v-group').forEach(g => {
                ...
                if (opts.length) selections.push({
                    name: vName,
                    options: opts
                });
            });
        ...
        function renderCart() {
            ...
            cartLines.forEach((ln, idx) => {
                ...
                const selHTML = (ln.selections || []).map(v => {
                    const opts = v.options.map(o => o.name + (o.adj ? ` (+${o.adj.toFixed(2)}${CURR})` :
                        ``)).join(', ');
            ...
            const srvEl = document.getElementById('t_srv');
            if (srvEl) srvEl.innerText = money(srv);
            const taxEl = document.getElementById('t_tax');
            if (taxEl) taxEl.innerText = money(tax);
        ...
        function setParked(a) {
            localStorage.setItem(PARK_KEY, JSON.stringify(a));
        }
            ...
            if (printIt) {
                ...
                summary.lines.forEach(l => {
                    ...
                    (l.selections || []).forEach(v => {
                        const opts = v.options.map(o => o.name + (o.adj ?
                            ` (+${o.adj.toFixed(2)}{{ $currency }})` : ``)).join(', ');
                        w.document.write(`<div class="small"> - ${v.name}: ${opts}</div>`);
                    });
                    w.document.write(
                        `<div class="line"><span class="small">{{ __('Unit') }}:</span><span>${l.unit.toFixed(2)}{{ $currency }}</span></div>`
                        );
                    w.document.write(
                        `<div class="line"><span class="small">{{ __('Line') }}:</span><span>${(l.unit*l.qty).toFixed(2)}{{ $currency }}</span></div><hr>`
                        );
                });
                w.document.write(
                    `<div class="line"><strong>{{ __('Subtotal') }}</strong><strong>${summary.totals.sub}</strong></div>`
                    );
                w.document.write(
                    `<div class="line"><strong>{{ __('Grand Total') }}</strong><strong>${summary.totals.grand}</strong></div><hr>`
                    );
                w.document.write(
                    `<div class="small">{{ __('Paid by') }}: ${summary.pay.method.toUpperCase()} • {{ __('Given') }}: ${summary.pay.given} • {{ __('Change') }}: ${summary.pay.change}</div>`
                    );
                w.document.write(
                    `<p class="small" style="text-align:center;margin-top:10px">*** {{ __('Thank you!') }} ***</p>`);
        ...
        scanModalEl.addEventListener('shown.bs.modal', () => {
            if (window.Html5QrcodeScanner) {
                qrScanner = new Html5QrcodeScanner("qrReader", {
                    fps: 10,
                    qrbox: 250
                });
