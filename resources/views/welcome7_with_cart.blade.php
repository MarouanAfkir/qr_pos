...
<head>
    ...
    <style>
        ...
        .hero {
            position:relative;
            background: radial-gradient(1200px 600px at 50% -30%, rgba(196,154,108,.35), transparent 55%),
        ...
        /* Lang + Theme toggles */
        .hero-tools { position:absolute; top:.75rem; {{ $isRTL ? 'left' : 'right' }}:.75rem; display:flex; gap:.5rem; }
        .btn-pill {
            background: rgba(255,255,255,.18);
            border:1px solid rgba(255,255,255,.6);
            color:#fff; padding:.35rem .65rem; border-radius:999px; font-weight:700; font-size:.9rem;
            backdrop-filter: blur(6px); transition: .2s;
        }
        .btn-pill:hover { background: rgba(255,255,255,.3) }
        .hero .dropdown-menu {
            border:none; border-radius:.75rem; padding:.35rem; min-width:12rem;
            background:#fff; box-shadow:0 16px 36px rgba(0,0,0,.2);
        }
        ...
        .restaurant-logo-wrapper{
            width:132px;height:132px;margin:0 auto .75rem;border-radius:50%;
            background:#fff;display:flex;align-items:center;justify-content:center;
            box-shadow:0 6px 18px rgba(0,0,0,.2);transition:.2s;border:3px solid rgba(255,255,255,.95);overflow:hidden
        }
        .restaurant-logo-wrapper:hover{transform:translateY(-2px);box-shadow:0 10px 26px rgba(0,0,0,.25)}
        .restaurant-logo-wrapper img{width:86%;height:86%;object-fit:contain;border-radius:50%}
        .restaurant-name{font-family:'Playfair Display',serif;font-size:2.1rem;font-weight:800;letter-spacing:.3px}
        .hero-meta{display:flex;gap:.9rem;justify-content:center;flex-wrap:wrap;font-size:.92rem;opacity:.95}
        ...
        [data-theme="dark"] .sticky-tools {
            background:rgba(23,23,23,.7); border-color:rgba(255,255,255,.06);
        }
        #menuSearch{
        ...
        #menuSearch:focus{border-color:var(--brand-2); box-shadow:0 0 0 6px var(--ring); outline:0}
        .quick-tags{display:flex; gap:.5rem; flex-wrap:wrap; justify-content:center; padding:.5rem 1rem 1rem}
        .quick-tags .qtag{
            border:1px solid var(--chip-b); background:var(--chip); color:#6b4b2f;
            padding:.25rem .6rem; font-size:.82rem; border-radius:999px; cursor:pointer; transition:.15s
        }
        ...
        .cat-nav{
            ...
            box-shadow:0 2px 10px rgba(0,0,0,.15); color:#7a5200; font-size:.85rem; z-index:5
        }
        .cat-nav.disabled{display:none}
        .cat-prev{{ $isRTL ? ':right' : ':left' }}:.35rem;
        .cat-next{{ $isRTL ? ':left' : ':right' }}:.35rem;

        .categories-scroll{
            display:flex; flex-wrap:nowrap; gap:.55rem; margin:.25rem 0 .55rem; padding:0 1.6rem; overflow-x:auto;
        ...
        .categories-scroll .nav-link{
            padding:.6rem 1.05rem; font-size:.95rem; font-weight:800; color:#5a3d1b;
            background:linear-gradient(145deg,#fff1de 0%,#fef1d0 100%); border:1px solid rgba(196,154,108,.45);
            border-radius:14px 14px 0 14px; transition:.18s box-shadow,.18s transform;
        }
        .categories-scroll .nav-link:hover{transform:translateY(-2px); box-shadow:0 6px 14px rgba(0,0,0,.08)}
        .categories-scroll .nav-link.active{color:#000; background:linear-gradient(135deg,#fcd9a7 0%,#f3c088 100%); border-color:transparent; box-shadow:0 4px 12px rgba(0,0,0,.1)}
        @media(min-width:576px){ .cat-nav{display:none} .categories-scroll{overflow:visible; flex-wrap:wrap; justify-content:center; gap:.75rem; padding:0} }

        /* ===== ITEMS ===== */
        .single-menu-items{
            cursor:pointer; display:flex; gap:1rem; padding:1rem; margin-bottom:1rem; background:#fff;
            border-radius:1rem; border:1px solid #efe6db; box-shadow:0 1px 4px rgba(0,0,0,.04); transition:.2s
        }
        .single-menu-items:hover{box-shadow:0 12px 26px rgba(0,0,0,.1); transform:translateY(-2px)}
        [data-theme="dark"] .single-menu-items{ background:#1e1e1e; border-color:#2b2b2b }
        .menu-item-thumb{width:92px; aspect-ratio:1/1; border-radius:14px; overflow:hidden; flex-shrink:0; background:#f3f4f6}
        .menu-item-thumb img{width:100%; height:100%; object-fit:cover; filter:saturate(1.05)}
        .menu-content h3{font-weight:800; font-size:1.05rem}
        .chips{margin-top:.35rem}
        .chip{display:inline-block; background:var(--chip); border:1px solid var(--chip-b); color:#6b4b2f; font-size:.72rem; padding:.15rem .45rem; border-radius:999px; margin:0 .25rem .25rem 0}
        .price-wrap del{color:#9ca3af; margin-inline-end:.35rem}
        .save-badge{background:#dcfce7; color:#065f46; border-radius:6px; font-size:.72rem; padding:.12rem .38rem; margin-inline-start:.35rem; font-weight:700}

        /* ===== MODAL ===== */
        #itemModal .modal-content{border:0; border-radius:1rem; box-shadow:0 10px 40px rgba(0,0,0,.25)}
        #itemModal .modal-header{
            background:linear-gradient(135deg,#fff1c6 0%,#fde68a 100%);
            border-bottom:none; border-top-left-radius:1rem; border-top-right-radius:1rem;
        }
        #itemModal .modal-title{font-weight:800; color:#5f3b0e}
        #itemModal .modal-body{padding:1.25rem 1.25rem 1.5rem}
        #itemModal img{width:130px; height:130px; object-fit:cover; border-radius:.8rem; box-shadow:0 6px 18px rgba(0,0,0,.12)}
        .lead-price{font-weight:800; color:var(--brand)}
        .v-group{margin:.65rem 0}
        .v-title{font-weight:800; margin-bottom:.35rem}
        .opt-chip{
            display:inline-flex; align-items:center; gap:.35rem; padding:.35rem .6rem; margin:.25rem .35rem .25rem 0;
        ...
        .opt-chip small{opacity:.8}
        .qty-wrap{display:flex; align-items:center; gap:.5rem}
        .qty-btn{width:34px; height:34px; display:flex; align-items:center; justify-content:center; border-radius:8px; border:1px solid var(--chip-b); background:var(--chip)}
        .qty-input{width:54px; text-align:center; border:1px solid var(--chip-b); background:#fff; border-radius:8px; padding:.35rem 0}

        .cta-row{display:flex; flex-wrap:wrap; align-items:center; gap:.75rem; justify-content:space-between; margin-top:.75rem}
        .btn-primary-cta{
            background:var(--brand); border:none; color:#fff; font-weight:800; padding:.6rem 1rem; border-radius:.7rem;
            box-shadow:0 10px 24px rgba(111,78,55,.25); transition:.15s
        }
        ...
        /* ===== TRAY (mini-cart) ===== */
        .tray{
            position:fixed; {{ $isRTL ? 'left' : 'right' }}:16px; bottom:16px; width:min(420px,92vw);
        ...
        [data-theme="dark"] .tray{ background:#1f1f1f; border-color:#2b2b2b }
        .tray.open{ transform: translateY(0) }
        .tray-header{display:flex; align-items:center; justify-content:space-between; padding:.75rem 1rem; background:linear-gradient(135deg,#f7e5cc,#fff)}
        .tray-body{max-height:45vh; overflow:auto; padding:.75rem 1rem}
        .tray-line{display:flex; gap:.75rem; padding:.55rem 0; border-bottom:1px dashed #e9e1d9}
        ...
        .tray-qty{display:flex; align-items:center; gap:.4rem}
        .tray-qty button{width:26px;height:26px;border:1px solid var(--chip-b); background:var(--chip); border-radius:6px}
        .tray-footer{display:flex; align-items:center; justify-content:space-between; padding:.75rem 1rem; background:#fffaf3; border-top:1px solid #eee2d4}
        .tray-toggle{
            position:fixed; {{ $isRTL ? 'left' : 'right' }}:16px; bottom:16px; width:54px; height:54px; border-radius:50%;
            background:var(--brand); color:#fff; border:none; display:flex; align-items:center; justify-content:center;
            box-shadow:0 12px 28px rgba(0,0,0,.25); z-index:71;
        }
        .tray-toggle .badge{
            position:absolute; top:-6px; {{ $isRTL ? 'left' : 'right' }}:-6px; background:#ef4444; color:#fff; border-radius:999px; padding:.15rem .4rem; font-size:.7rem; font-weight:800
        }

        /* Back to top */
        #toTop{position:fixed; {{ $isRTL ? 'left' : 'right' }}:90px; bottom:18px; width:44px;height:44px;border-radius:50%;background:var(--accent);color:#222;border:none;box-shadow:0 8px 22px rgba(0,0,0,.18);display:none;align-items:center;justify-content:center;z-index:60}
        #toTop:hover{background:#ffe16a}

        /* Footer */
        .simple-footer{background:#0b0b0b; color:#c7c7c7; font-size:.9rem; padding:1.2rem 0}
        .simple-footer a{color:#fff; font-weight:800; text-decoration:none}
...
<body data-theme="light">
    ...
    <section class="hero">
        <div class="hero-inner">
            ...
            <div class="hero-tools">
                {{-- Theme toggle --}}
                <button id="themeToggle" class="btn-pill" title="{{ __('Toggle theme') }}">
            ...
            <p class="mb-2 fst-italic">{{ $tagline }}</p>
    ...
    <div class="sticky-tools">
        <div class="container py-2">
            ...
            {{-- Quick suggestion tags (autofill search) --}}
    ...
    <section class="food-menu-section fix section-padding">
        <div class="food-menu-wrapper style1">
            <div class="container ">
                <div class="food-menu-tab-wrapper style-bg pt-3 px-1 px-md-5">
                        ...
                        <div class="flex-grow-1">
                            ...
                            <div class="cta-row">
                                <button id="addToTray" class="btn-primary-cta">
                                    <i class="fa-solid fa-cart-plus me-1"></i> {{ __('Add to Tray') }}
                                </button>
                                <div class="small text-muted" id="perUnitNote">{{ __('Price per unit including selected options') }}</div>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    {{-- ===== Tray (Mini-cart) ===== --}}
    <button class="tray-toggle" id="trayToggle" title="{{ __('Open order') }}">
        <i class="fa-solid fa-bag-shopping"></i>
        <span class="badge" id="trayCount">0</span>
    </button>
    <div class="tray" id="tray">
        <div class="tray-header">
            <strong><i class="fa-solid fa-receipt me-2"></i>{{ __('Your Order') }}</strong>
            <button class="btn btn-sm btn-outline-secondary" id="clearTray"><i class="fa-solid fa-trash-can"></i> {{ __('Clear') }}</button>
        </div>
        <div class="tray-body" id="trayBody"></div>
        <div class="tray-footer">
            <div>
                <div class="fw-bold">{{ __('Total') }}: <span id="trayTotal">0{{ $currency }}</span></div>
                <small class="text-muted">{{ __('Taxes & fees may apply at checkout') }}</small>
            </div>
            <button class="btn btn-success fw-bold"><i class="fa-solid fa-check me-1"></i>{{ __('Proceed') }}</button>
        </div>
    </div>
    ...
    <script>
        ...
        /* ===== Modal logic: build interactive variations ===== */
        const itemModal = new bootstrap.Modal(document.getElementById('itemModal'));
        let currentItem = null; // store state for add-to-tray
        ...
        function renderVariations(variations){
            ...
            variations.forEach((v, vIdx) => {
                ...
                (v.options||[]).forEach((o, oIdx) => {
                    ...
                    const adjTxt = adj ? `<small>(+${formatMoney(adj).replace('{{ $currency }}','')}}{{ $currency }})</small>` : '';
        ...
        function recalcPrice(){
            ...
            document.getElementById('perUnitNote').style.opacity = 1;
        ...
        function enforceMax(groupEl, clickedChip){
            ...
            if(single){
                ...
                if(clickedChip.classList.contains('active')){
                    // allow untoggle always (but ensure min will be validated elsewhere)
        ...
        function applyDefaults(){
            document.querySelectorAll('#modalOptions .v-group').forEach(group=>{
                ...
                if(defaults.length){
                    if(single){
                        defaults.forEach((c,i)=> c.classList.toggle('active', i===0)); // pick first default
                    }else{
                        defaults.forEach(c=> c.classList.add('active'));
                    }
                }else{
                    // if required & only one option, select it
                    const min = parseInt(group.getAttribute('data-min')||0,10);
        ...
        function getTray(){ try { return JSON.parse(localStorage.getItem('tray')||'[]'); } catch(e){ return []; } }
        function setTray(arr){ localStorage.setItem('tray', JSON.stringify(arr)); renderTray(); }

        function addToTray(line){
            const trayArr = getTray();
            trayArr.push(line);
            setTray(trayArr);
            // open tray and animate
            tray.classList.add('open');
            trayToggle.querySelector('.badge').classList.add('animate__animated','animate__heartBeat');
            setTimeout(()=> trayToggle.querySelector('.badge').classList.remove('animate__animated','animate__heartBeat'), 1200);
        }
        ...
        clearTray.addEventListener('click', ()=> setTray([]));
        renderTray();
