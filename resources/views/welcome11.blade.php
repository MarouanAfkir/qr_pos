...
<head>
  ...
  <style>
    :root {
      --primary: #D35400; /* orange used across UI */
      --accent:  #F39C12;
      --light:   #FFF8F1;
      --dark:    #2C3E50;
      --ring:    rgba(211,84,0,.25);
    }
    * { box-sizing: border-box; }
    ...
    /* Better dropdown design (white, soft shadow, rounded) */
    ...
    .card-body p { font-size:.85rem; color:#666; height:2.5em; overflow:hidden; margin:0; }
    .card-footer { background: var(--light); padding:1rem; display:flex; justify-content:space-between; align-items:center; }
    .price { font-weight:600; color:var(--primary); }
    .btn-order { background:var(--primary); color:#fff; border:none; padding:.5rem 1rem; border-radius:50px; font-size:.85rem; transition:.2s; }
    ...
    /* Make modal price orange like cards */
    #modalPrice { color: var(--primary); }

    .detail-item { margin-bottom:1rem; font-size:.95rem; }
    .detail-item strong { color:var(--dark); }

    /* No results text */
    #noResults { display:none; text-align:center; color:#777; padding: 1rem 0 2rem; }

    footer { text-align:center; padding:1rem; font-size:.875rem; color:#555; background:#fff; box-shadow: 0 -2px 8px rgba(0,0,0,.05); }
  </style>
</head>
<body>
  ...
  <script>
    ...
    function filterBySearch(query) {
      ...
      sections.forEach(section => {
        // For each card within a section, check text
    ...
    catButtons.forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        // If already active -> clear selection (show all)
        if (isActive) {
          showAllSections();
          return;
        }
    ...
    searchInput.addEventListener('input', e => {
      ...
      if (!q.length) {
        // when clearing search: if a category is selected show it, else all
    ...
    document.querySelectorAll('.btn-order').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        document.getElementById('modalPrice').innerHTML = btn.dataset.price; // color handled via CSS
